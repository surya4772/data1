...
$site_header = '/var/gerrit/review_site/etc/GerritSiteHeader.html';
...
sub add_review_link {
  ...
  my $q;
  if (!$h || $h eq 'HEAD') {
    $q = qq{#q,project:$ENV{'GERRIT_PROJECT_NAME'}};
  } elsif ($h =~ /^refs\/heads\/([-\w]+)$/) {
    $q = qq{#q,project:$ENV{'GERRIT_PROJECT_NAME'}+branch:$1};
